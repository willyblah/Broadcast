...
| action | 其他请求参数 | 结果 |
| --- | --- | --- |
| `send` | `request_id`, `body`, `classrooms: string[]`, 可空 `source_id` | `id` |
...
`pending_broadcasts.items` 每项包含 `delivery_id`, `broadcast_id`, `body`, `created_at`, `expires_at`。查询不传入设备 ID，由服务端从 JWT 确定身份。教室客户端根据 `body` 直接调用腾讯云 TTS，并在本机播放生成的 WAV。
...
教室收到订阅成功后开始上报心跳；随后每 20 秒发送 WebSocket heartbeat 并等待确认，再上报设备心跳。服务端超过 50 秒没有收到有效心跳时将教室标记为离线。重连自动退避至约 10 秒，并在连接恢复后查询未过期消息。每轮正常心跳同时补查有效投递，避免单条通知丢失。
