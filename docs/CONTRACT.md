...
| action | 其他请求参数 | 结果 |
| --- | --- | --- |
| `preview` | `body` | `audio_id`, `url`, `expires_at` |
| `send` | `request_id`, `body`, `classrooms: string[]`, 可空 `source_id` | `id`, 可空 `tts_error` |
| `register-device` | `classroom_id`, `name` | Supabase `session`, `classroom_id` |
| `audio-url` | `delivery_id` | `audio_id`, `url`, `expires_at` |

前三项仅管理员可用。`audio-url` 允许管理员或当前有效绑定且拥有投递的设备使用。语音合成失败时 `preview` 返回错误，`send` 仍创建文字广播。创建请求以 `request_id` 去重；同一逻辑重试必须保留该 ID，明确重新发送使用新 ID。
...
`create_broadcast` 供发送云函数在管理员身份下调用，参数为 `p_id`, `p_body`, `p_classrooms`, `p_audio`, `p_tts_error`, `p_source`，一次事务写入广播和所有班级投递。

`pending_broadcasts.items` 每项包含 `delivery_id`, `broadcast_id`, `body`, `created_at`, `expires_at`, `audio_id`, `tts_error`。查询不传入设备 ID，由服务端从 JWT 确定身份。
...
教室收到订阅成功后开始上报心跳；随后每 10 秒发送 WebSocket heartbeat 并等待确认，再上报设备心跳。超过 25 秒没有心跳，老师端判离线。重连自动退避至约 10 秒，并在连接恢复后查询未过期消息。每轮正常心跳同时补查有效投递，避免单条通知丢失。
