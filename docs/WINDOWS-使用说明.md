...
- 收到有效广播后，全屏显示发布老师、可选 emoji 和鲜明颜色的正文，由本机直接调用腾讯云生成并播放语音。
- 语音可设置为 0–5 遍。0 遍只显示文字；多遍播放只合成一份音频，由本机重复播放。
- 选择自动关闭时，正常音频播完后停留 3 秒；选择手动关闭时，界面保留并显示“关闭”按钮。在手动关闭前，后续广播仍按顺序排队，可能因 30 秒有效期超时。
