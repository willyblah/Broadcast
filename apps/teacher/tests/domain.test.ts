...
const room: Classroom = { id: '8-1', device_id: 'one', device_name: 'PC', connected: true, last_seen_at: new Date(now - 24_999).toISOString() };
...
const b: Broadcast = { id: 'one', body: '测试', created_at: new Date(now - 30_000).toISOString(), expires_at: new Date(now).toISOString(), audio_id: null, tts_error: null, source_id: null, deliveries: [d] };
describe('truthful device status', () => {
  it('expires the heartbeat at exactly 25 seconds', () => { expect(isOnline(room, now)).toBe(true); expect(isOnline(room, now + 1)).toBe(false); });
