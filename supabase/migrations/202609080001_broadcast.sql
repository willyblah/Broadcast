...
begin
  ...
  return jsonb_build_object('server_now', clock_timestamp(), 'classrooms', (
    select jsonb_agg(jsonb_build_object('id', c.id, 'device_id', c.device_id, 'device_name', d.name,
      ...
      'connected', coalesce(d.connected and d.last_seen_at > clock_timestamp() - interval '50 seconds', false)) order by c.id)
    from public.classrooms c left join public.devices d on d.id = c.device_id
  ));
end $$;

create function public.create_broadcast(p_id uuid, p_body text, p_classrooms text[], p_source uuid default null) returns uuid
language plpgsql security definer set search_path = '' as $$
...
begin
  ...
  insert into public.deliveries(broadcast_id, classroom_id, device_id, online_at_send)
    select p_id, c.id, c.device_id, coalesce(d.connected and d.last_seen_at > v_created - interval '50 seconds', false)
...
grant select on public.classrooms, public.devices, public.deliveries, public.broadcasts to authenticated;
...
grant execute on function public.is_admin(), public.owns_device(uuid), public.device_classroom(), public.bind_device(uuid,text,text),
  ...
  public.create_broadcast(uuid,text,text[],uuid), public.pending_broadcasts(), public.start_delivery(uuid),
